...
function preload()
{
  ...
  scoreImg = loadImage("empty.png");
  fullImg = loadImage("stars.png");
...
function setup()
{
  ...
  star1 = createSprite(width/2 + 30, 100);
  star1.addImage(starImg);
  star1.scale = 0.02;

  star2 = createSprite(100, height/2 + 20);
  ...
  scoreBoard = createSprite(50, 20);
  scoreBoard.addAnimation("score", scoreImg);
  scoreBoard.addAnimation("scoreFull", fullImg);
  scoreBoard.scale = 0.2;

  blower = createImg("baloon2.png");
  blower.size(100, 120);
  blower.position(width/2 - 30, height/2);
...
function draw()
{
  ...
  if(fruit!=null && fruit.position.y>=650)
  {
   ...
   if(collide(fruit, star1, 20)) {
     star1.visible = false;
     score1 = createSprite(75, 20);
     score1.addImage(starImg);
     score1.scale = 0.02;
   }

   if(collide(fruit, star2, 20)) {
     star2.visible = false;
     scoreBoard.changeAnimation("scoreFull");
   }
...
function blow() {
  Body.applyForce(fruit, {x: 0, y: 0}, {x: 0, y: -0.05});
}
...
function collisionDetection(body, sprite, size) {
